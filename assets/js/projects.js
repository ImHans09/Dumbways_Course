const projects = [
  {
    ...
    "imagePath": "/assets/images/sample_academic_information_system.png"
  },
  {
    "id": 2,
...
function submitNewProject(form) {
  ...
  const projectImageInput = form.querySelector('input[name="imagePath"]');

  const startDateAsNumber = projectStartDateInput.valueAsNumber;
  const endDateAsNumber = projectEndDateInput.valueAsNumber;
  const projectDateObject = new Date(projectEndDateInput.value);
  const projectYear = projectDateObject.getFullYear();
...
// Method for saving selected project data to localstorage
function saveProjectData(id) {
  const project = projects.find(project => project.id === id);
  localStorage.setItem('selectedProject', JSON.stringify(project));
  window.location.href = `/detail_project.html?id=${id}`
}

// Method for creating project card
function createProjectCard(project) {
  ...
  projectCard.innerHTML = `<div id="projectCard" class="card shadow">
            <a href="#" onclick="saveProjectData(${project.id})">
              <img src="${project.imagePath}" class="card-img-top p-2 rounded-4" alt="Photo Profile">
            </a>
            <div class="card-body d-flex flex-column gap-3">
              <div class="d-flex flex-column gap-2">
                <div class="d-flex flex-column">
                  <a href="#" onclick="saveProjectData(${project.id})" class="card-title m-0">${project.name}</a>
                  <span>${project.year}</span>
                </div>
                <span>Duration: ${project.duration} day(s)</span>
              </div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  addProjectForm.addEventListener('submit', (event) => {
    ...
    projects.map(project => projectCardContainer.appendChild(createProjectCard(project)));
